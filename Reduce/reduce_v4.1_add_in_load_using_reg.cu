...
__global__ void reduce_add_in_load_using_reg(float *d_input, float *d_output)
{
    ...
    float sum = shared[threadIdx.x];
    for (int i = blockDim.x / 2; i > 0; i /= 2)
    {
        if (threadIdx.x < i){
            sum += shared[threadIdx.x + i];
...
int main()
{
    ...
    for (int i = 0; i < 10; i++)
        reduce_add_in_load_using_reg<<<Grid, Block>>>(d_input, d_output);
